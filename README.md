...
- 使用滚动 Median/MAD 基线识别磁盘和网络流量异常；
- 连续确认应用进程组的内存增长，提示疑似缓存积累或内存泄漏；
- 异常具备恢复迟滞和 10 分钟告警冷却，减少重复提醒；
- 持久化记录应用启动、最后观察、关闭时间和总运行时长；
- 通过 Windows 前台窗口归属累计实际前台使用时间，并区分后台时长；
- 提供应用使用记录页面，显示运行状态、会话和进程成员峰值；
